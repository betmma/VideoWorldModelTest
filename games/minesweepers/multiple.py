...
class MultipleMinesweeper(MinesweeperBase):
            ...
            for c in range(self.grid_w):
                ...
                for nr, nc in self.get_adjacent(r, c):
                    if self.grid[nr][nc].is_mine:
                        clue += 2 if self.colored[nr][nc] else 1
    ...
    def get_active_constraints(self, revealed_state, flagged_state):
        ...
        for r in range(self.grid_h):
            for c in range(self.grid_w):
                if revealed_state[r][c] and self.grid[r][c].clue > 0:
                    adj = self.get_adjacent(r, c)
                    weights = {}
                    for nr, nc in adj:
                        weights[(nr, nc)] = 2 if self.colored[nr][nc] else 1
