...
class ThreeWayMatchOnet(OnetBase):
    """Onet variant where three identical tiles must be connected through two segments."""
    ...
    def get_rule_prompt_text(self) -> str:
        """Describe the three-way matching rule for the training prompt."""
        return "Match three identical tiles. The connection must go from the first tile to the middle tile and then to the last tile, and each segment cannot bend more than twice."
    ...
    def try_create_match(self, positions: list[tuple[int, int]]) -> MatchResult | None:
        """Accept any order of three identical tiles whose two segments each use at most two turns."""
...
if __name__ == "__main__":
    ...
    run_autoplay(ThreeWayMatchOnet)